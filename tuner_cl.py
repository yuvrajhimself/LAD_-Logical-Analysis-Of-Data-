...
def run_with_timeout(fn, args, kwargs, timeout_s):
    ...
    Returns (result, timed_out, exception).
      ...
      - If timeout:              (None,   True,  None)
      - If exception:            (None,   False, exc)
    Note: the background thread cannot be truly killed in Python.
    After timeout the thread keeps running in the background but
    we stop waiting for it — the next trial starts immediately.
    """
    result    = [None]
    exc_box   = [None]
    ...
    t.start()
    finished = completed.wait(timeout=timeout_s)

    if not finished:
        return None, True, None       # timed out
    if exc_box[0] is not None:
        return None, False, exc_box[0]  # exception
    return result[0], False, None     # success
        ...
        for s_idx, s_cfg in enumerate(SELECTOR_GRID):
            ...
            for m_cfg in MINER_GRID:
                ...
                try:
                    ...
                    # Run fit inside a timeout-protected thread
                    def _fit():
                        silent(miner.fit, Xsel_train, y_clean,
                               original_feature_names=feature_names)

                    _, timed_out, fit_exc = run_with_timeout(
                        _fit, [], {}, TRIAL_TIMEOUT_SECONDS
                    )
